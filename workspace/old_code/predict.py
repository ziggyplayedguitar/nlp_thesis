...
from preprocess_data import TweetPreprocessor
import argparse
...
class TrollPredictor:
    ...
    def prepare_input(self, tweets: List[str]) -> Dict[str, torch.Tensor]:
        ...
        if len(processed_tweets) < self.tweets_per_account:
            # Repeat tweets to reach desired count
            processed_tweets = (processed_tweets * ((self.tweets_per_account // len(processed_tweets)) + 1))[:self.tweets_per_account]
        elif len(processed_tweets) > self.tweets_per_account:
            # Take only the first tweets_per_account tweets
            processed_tweets = processed_tweets[:self.tweets_per_account]
    ...
    def predict(self, tweets: List[str]) -> Dict[str, Union[str, float]]:
        ...
        with torch.no_grad():
            outputs = self.model(
                ...
                tweets_per_account=self.tweets_per_account
            )
    ...
    def explain_prediction(self, tweets: List[str]) -> Dict[str, List[Dict]]:
        ...
        # Process each tweet
        for i in range(min(self.tweets_per_account, len(processed_tweets))):
            print(f"\nProcessing tweet {i+1}/{min(self.tweets_per_account, len(processed_tweets))}")
...
def main():
    parser = argparse.ArgumentParser(description='Predict if tweets are from a troll account')
    parser.add_argument('--model_path', type=str, required=True, help='Path to the trained model')
    parser.add_argument('--input_file', type=str, help='Path to JSON file containing tweets')
    parser.add_argument('--tweets', nargs='+', help='List of tweets to analyze')
    parser.add_argument('--device', type=str, default=None, help='Device to run on (cuda or cpu)')
    parser.add_argument('--explain', action='store_true', help='Enable explanation')
    parser.add_argument('--explain_method', type=str, default='occlusion', 
                       choices=['occlusion', 'correlation'], 
                       help='Method for generating explanations')

    args = parser.parse_args()
    
    # Initialize predictor
    predictor = TrollPredictor(args.model_path, device=args.device)
    
    # Get tweets either from file or command line
    if args.input_file:
        ...
        raise ValueError("Either --input_file or --tweets must be provided")
    ...
    if 'attention_weights' in result:
        print("\nTweet Importance Weights:")
        for i, (tweet, weight) in enumerate(zip(tweets[:predictor.tweets_per_account], result['attention_weights'])):
            print(f"\nTweet {i+1} (Weight: {weight:.3f}):")
            print(tweet)

    # If explanation is requested
    if args.explain:
        print("\n🔍 Generating explanations...")
        if args.explain_method == 'correlation':
            explanation_data = predictor.explain_with_correlation(tweets)
        else:
            explanation_data = predictor.explain_prediction(tweets)
